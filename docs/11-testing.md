...
| **`runBlueprint` executor** (executor / rules / template / slots / verify / `when` / corpus-loader full-DAG) | D89/D56/D59a/D45/D67/D49/D10 | **BUILT Session 13** (executor + rules + verify + slots suites + adversarial qa4/qa5/qa6). Scope-honesty gate fail-closed at load (`SELECT *`/alias-mask/JOIN-to-unlisted/`dictGet` bypasses closed); D56 no-unverified-return (verify-fail/unmappable/probe-error all withhold; `grain_verifiable:false` skips visibly); scalar-consumed node `!=1` row/col ⇒ fail-closed; slot + scalar + `resolve_via` values **bound as typed AST literals, never interpolated** (D10); table-intermediate **materialized to session scratch + JOIN AST-rewritten** when a `scratch_client` is wired, rejected pre-dispatch (`RUN_BLUEPRINT_UNSUPPORTED` → raw loop) only when it is NOT (F2, closed by D93 — `test_executor_table_intermediate.py` + `_qa2`; truncated/oversized/no-column intermediates and un-materialized resume all fail closed); D45 approval pause/resume CAS-exactly-once + restart-durable; resumed provenance union fail-closed across the pause; approval affirmative-only (no fail-open consent); `resolve_via` empty ⇒ fail-closed / degraded ⇒ raw-loop; 1 `tool_calls_made` per `runBlueprint`; non-oracle `getBlueprint` preserved for full-DAG blueprints |
| **Retrieval stack + read tools** (pipeline / neo4j index / corpus loader / registry / 3 read tools) | D86/D87/D88/D44/D25 | **BUILT Sessions 10–12** (~250 adversarial cases + live Layer-2). Byte-identical Phase-0 parity when unwired; degrade-not-fail at every stage, observably; render structure-sanitized; `uses` byte-exact fail-closed at write AND read; model-parity strict-write/degrade-read; non-oracle `getBlueprint`; footprint-split provenance (FOUND getBlueprint drops under narrowing); `query`/question redacted from all telemetry (real-OTel e2es); live proofs vs real neo4j+embedder+reranker (semantic ordering, scope drop on stored USES, round-trips) |

## Layer 2 — Component / Integration (one component + its real store)

Run against a **real** containerized store, not the full stack:

- **MCP data-plane** (vs. ClickHouse): read-only enforced (INSERT/UPDATE/DELETE/DDL/table-functions blocked), row caps + time limits, **scope rejection by SQL parse (D57)**, `getTableSchema` returns the **merged + scope-filtered schema (D83, reverses D78's introspection-only framing)**, `sampleRows` is **column-scope-enforced by reject (D83)**.
- **MCP catalog overlay** (vs. catalog + MCP, formerly "Runtime catalog overlay"): the **MCP** performs `introspection ⨝ catalog overlay` and scope-filters the result, incl. the uncatalogued-table structural-only path (D53/D83, D83 reverses D78's runtime-side placement) — this is now an **MCP** component test (MCP ↔ ClickHouse ↔ catalog), not a runtime one. Also covers `sampleRows`'s scope-rejection behavior as an MCP-enforced case (previously unscoped — see WORKLOG.md gap note).
- **`runBlueprint` executor** (vs. ClickHouse + neo4j): **BUILT + live-green Session 13 (D89) — single- AND multi-node scalar DAGs verified end-to-end vs real neo4j + ClickHouse-via-MCP (3/3); non-oracle `getBlueprint` NOT_FOUND for a narrow scope.** Topo-sort DAG walk (D33 no branching); **scalar→typed AST literal (D59a/F1 — reframed from "server-side param"; table→scratch BUILT via D93 materialize-and-join, gated on a wired `scratch_client` and rejected pre-dispatch → raw loop without one)**; approval gate references upstream-only (D59b); **pause checkpoint + CAS-exactly-once resume, restart-durable (D45)**; verification gate runs + withholds on failure (D56).
...
> **runBlueprint Layer-3 status (D89, Session 13):** the four scenarios this brick unlocks —
> **No-silent verification** (D56 wrong-grain caught), **Ask → clarify** (slot ambiguity), **Scope
> denial** (out-of-scope blueprint non-oracle), and **Pause/resume durability** (mid-DAG approval
> restart) — are **authored and Layer-1/2-proven** (unit + live single/multi-node DAG vs real
> neo4j + ClickHouse-via-MCP), but the **Playwright demo-wiring is deferred** (not yet demo-green).
> They remain 🟡 until the Layer-3 harness drives them through the real UI.

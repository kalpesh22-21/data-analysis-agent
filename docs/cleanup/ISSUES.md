...
ONE behaviour change: `_blueprint_usages` now skips infra failures as well as gates, so an outage emits NO usage (not `accepted` — that would be the opposite conflation). `_failed_fixed_pairs` is UNCHANGED and still pairs them, and that asymmetry is the point: an outage is genuinely analyst friction (they hit a wall and re-ran) while being no evidence at all about the blueprint's SQL, and one boolean could not say both. Metric movement is prospective only — `SessionSignals` is stamped at envelope-build, so no existing inbox row re-scores. A second patch registers the six codes that reached the trail unregistered (`RUN_BLUEPRINT_NOT_FOUND` / `_SLOT_INVALID` / `_VERIFY_FAILED` as `WORK_JUDGED`, `_UNSUPPORTED` / `_ABORTED` as `GATE`, `INTERNAL_TRANSPORT_ERROR` as `INFRA_FAILED`), which moves metrics deliberately: an unregistered code took the `classify_denial` fallback, so a fast path that DECLINED a blueprint on capability and a USER who denied an approval gate were both recorded as blueprints that produced wrong answers. Each entry also fixes the adjacent latent replay bug — `context/budget.py::_render_entry` was rendering the generic "Something went wrong processing that request." for all six — with the executor's own live text, spelled rather than imported (`executor.py` → `tool_dispatcher.py` → this table is a cycle) and pinned byte-identical by a test that can import both sides. Still unregistered and worth a later look: `RUN_BLUEPRINT_INVALID_ARGS` / `RUN_BLUEPRINT_INTERNAL_ERROR` (`blueprint/tool.py`) and `RUN_BLUEPRINT_UNAVAILABLE` (`loop/agent_loop.py`).
...
**K2 — G1 LANDED; G2 deferred for prompt headroom.** The original entry was "L5 red 0/3, model never declares intents". That was closed by the model, not the code: under gpt-5.5 the model declares intents unprompted and L5 re-baselined **3/3** (WORKLOG #5). The residual was that the drop stayed *silent* — when the model tags `serves_intent` before any state is live, `split_serves_intent` drops the tag (`loop_intent_tag_dropped{no_live_state}`, degrade-not-fail) and nothing told the model, so the failure mode was latent behind a model behaviour that could regress with any model change.

**G1 is now closed by construction.** The drop site (`loop/agent_loop.py`, the dispatch body) selects the tagged call and the NEXT rebuild renders `_INTENT_TAG_DROPPED_NOTE` onto that call's tool result under its own `runtime_note` key — never `note`/`window_note`, which carry independent claims. Ephemeral, never persisted, EXACTLY ONE ROUND-TRIP (the same lifetime as `finalization_nudge`), at most ONE note per round however many tags a batch drops, and bound to `no_live_state` alone — for `unknown_intent_id`/`not_a_string` a state DOES exist and "declare your intents" would be false advice.

**The note is GATED on the late-init door still being open** (review blocker, fixed before landing). `runQuery(serves_intent=…)` with no state drops the tag for `no_live_state` *and is itself substantive*, so the call that earns the note also locks late init — and a model that then complied would get a NON-RETRYABLY refused `updateAnalysisState`, i.e. an instructed dead end worse than the silence. Same shape via a mixed batch `[tagged getTableSchema, untagged runQuery]`. So the loop tracks `substantive_ran` and **clears the selection at the END of the dispatch batch** (end-of-batch, not at the drop site, because within one batch the tagged and the substantive call arrive in either order). The flag is **TURN-scoped and seeded** via `find_locking_tool` from the session doc the window already loads — a plain window-local would read False again after a budget-cap `continue`/askUser/blueprint resume while the trail still had the door shut. Suppressed, not reworded: with the door shut no true corrective advice exists, so the pre-slice status quo (silent drop) is the right fallback. Telemetry is unchanged throughout: every drop still emits `loop_intent_tag_dropped`.

The prompt gained its half at +68 chars ("DECLARE THEM FIRST with updateAnalysisState" and "a serves_intent tag sent before any declaration is IGNORED"). Tests: `tests/runtime/loop/test_intent_tagging_loop.py` (11 — delivery/exact text/one-round-trip, own-key isolation, live-state drop gets no note, the no-drop negative, the once-per-round rule, the three suppression shapes (tagged-substantive, mixed batch, earlier-round query), the seed across a pause, that the advice is TRUE where it does fire (`getTableSchema` is outside `SUBSTANTIVE_TOOLS`, so the late declaration is accepted), and that the resulting late declaration still finalizes through the pending-intents gate instead of hard-locking the turn).

**G2 (decomposition trigger under-fires on non-enumerated conjunctive questions, "X and Y" vs "I need three things:") stays OPEN, and is now explicitly size-blocked.** Its prompt line — the conjunctive example, "the employee columns, and the average salary by department" is TWO deliverables — drafts at +150 chars against 86 remaining under the 17,400 ceiling after G1. Shaving it to ~78 fits arithmetically and leaves 8 chars spare, below the 21 the ceiling ledger already called "no headroom at all", and the shave deletes the example, which IS the instruction. So it waits for a ceiling argument of its own rather than shipping shaved; the reasoning is recorded in the ledger docstring of `tests/runtime/test_prompt_routing_contract.py::test_prompt_stays_within_its_token_budget`. **Still size-blocked after the 2026-08-28 raise to 17,550** (the date-arithmetic rule): 76 spare, so the arithmetic is unchanged and G2 still needs a ceiling argument of its own. G3 (report state-None distinctly) landed with WORKLOG #5. **Do NOT relax late-init.**

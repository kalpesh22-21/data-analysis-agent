...
**H8 — outage codes strain the binary enforcement taxonomy (2026-08-17, H1 review nit).** `DenialInfo.enforcement` is a boolean serving two readers with different semantics: `CLICKHOUSE_UNAVAILABLE` (enforcement=False) on a `runBlueprint` makes `_blueprint_usages` infer `outcome="corrected"` — "this blueprint was wrong" from a warehouse outage. The real shape is three-way (gate / work-judged / infra-failed) collapsed into two. Pre-existing behaviour, deliberately preserved by the H1 slice (changing it moves a metric); whoever extends the split must know the boolean is load-bearing for both readers.

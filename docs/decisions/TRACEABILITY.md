...
| **D59** | Composite blueprint inter-node **table** outputs — ~~DEFERRED (F2, D89): no scratch-write surface exists yet, so a table-intermediate DAG is **rejected pre-dispatch** (scalar-converging only)~~ **CLOSED by D93 (2026-08-18, ISSUES D1):** materialized to session scratch + `JOIN` AST-rewritten **when a `scratch_client` is wired**; pre-dispatch rejection → raw loop only when it is not. See the D93 / D59a row below | Unit + Component | `D59a-table-intermediate-to-scratch` | — | 🟡 unit-green (both arms proven: materialize-and-join AND the unwired rejection) |
...
| **D89 / F2** | A table-intermediate DAG is **rejected pre-dispatch** (`RUN_BLUEPRINT_UNSUPPORTED` → raw loop) — **conditional since D93 (2026-08-18, ISSUES D1): only when no `scratch_client` is wired.** With one wired it materializes and JOINs (D93 / D59a row above) | Unit | `D89-table-intermediate-rejected` | — | 🟡 unit-green |
...
| **D97** | Role: a user-parameter literal → required slot bound to the column domain | Unit | `S3-role-slot-user-parameter` | — | ⛔ not-built |
| **D97 / D35** | Role: a catalog-rule-resolvable literal → `rule` with the existing `rule_id`, no literal in the plan | Unit | `S3-role-rule-catalog-resolvable` | — | ⛔ not-built |
| **D97** | Role: a metric-defining literal (`record_type='EARNING'`) → `inline`, stays literal | Unit | `S3-role-inline-metric-defining` | — | ⛔ not-built |
| **D97** | Caller-specific literal (`region='NA'`) → optional slot (`required:false` + `optional_pattern`), NOT deleted (no drop) | Unit | `S3-caller-specific-to-optional-slot` | — | ⛔ not-built |
| **D97** | Adversarial mis-classification: a metric-defining predicate offered as optional-slot without an `optional_pattern` is rejected to review | Unit | `S3-adversarial-role-misclassification` | — | ⛔ not-built |
| **D97** | No-drop totality: every literal predicate of the accepted SQL has exactly one `ParamPlan`; a missing entry fails validation (fail-to-review) | Unit | `S3-no-drop-totality` | — | ⛔ not-built |
| **D97** | Unclassifiable / missing catalog rule → fail-to-review, never auto-promoted | Unit | `S3-unclassifiable-fails-to-review` | — | ⛔ not-built |
| **D31** | Evidence mandatory: a candidate with empty `evidence` is REJECTED at emit (no audit snapshot, no persist) | Unit | `S3-no-evidence-rejected` | — | ⛔ not-built |
| **D34** | Lift-not-generate: `accepted_signal` required; a no-acceptance session yields no blueprint candidate; the plan derives only from the accepted SQL | Unit | `S3-lift-not-generate-accepted-signal` | — | ⛔ not-built |
| **D52** | Un-AST-rewritable / un-parseable accepted SQL → fail-to-review, never a guessed template | Unit | `S3-unrewritable-sql-fails-to-review` | — | ⛔ not-built |
| **D31** | Forced structured output: the extractor requires the `emit_candidates` tool call and RETRIES on a malformed (non-tool-call) response; the model client is injected (scripted double in tests) | Unit | `S3-structured-output-retry` | — | ⛔ not-built |
...
[learning-loop-wave3-wiring-design.md](learning-loop-wave3-wiring-design.md). **Update (Session 20 cont.,
Wave 3b-ii, commit `e520b33`): the Layer-2 legs are now GREEN against real infra** — docker stack up
(real Couchbase + Redis + neo4j), `learning_user` provisioned via `scripts/learning-user-init.sh` (all
four learning buckets init'd), **39/39 learning Layer-2 tests green**, each `*_live.py` run in its own
process (the D101 Couchbase C-ext segfault guard). The durable corpus, consumer-wiring, and the
**fully-wired pipeline** rows are now **Layer-2-proven** (`✅ green` for the L2 leg, marked below);
they read `🟡 unit-green` where a downstream leg is still deferred. The loop is production-activatable
but **dormant** (`LEARNING_ENABLED` off, no request-path import). **Layer-3 conformance for the learning
loop is a separate later concern, and auto-promotion is still gated on the deferred real
`WarehouseProbe`/`DependencyResolver` + GitHub PR client + the S9 corpus-landing writer.**

| Decision | Invariant | Test layer | Test slug | Conformance scenario | Status |
|---|---|---|---|---|---|
| **D48** | The durable blueprint corpus increments `hit_count` **atomically** via a single Couchbase `mutate_in` counter op (never a read-modify-write), so concurrent identical-semantics merges yield one create + N atomic bumps, never a lost update; the seed is **insert-wins idempotent** (a re-seed of an existing `canonical_key` never resets its count); it duck-types the S9 `HitCountReader` | Unit + Component | `wave3-corpus-atomic-hit-count` | — | ✅ green (Layer-2 live vs real Couchbase: atomic concurrent-increment +2, insert-wins seed, RBAC boundary — Session 20 cont., `e520b33`) |
| **D102** | `build_learning_consumer` assembles the six write-router stages in the **frozen order** (`generalize→leakage→dedup→schema_edit_pr→user_commit→writer`) over shared-singleton stores under **all-or-nothing** config gating (a partial wiring config refuses to assemble); a session runs end-to-end through every wired stage AND the S1 spine invariants (kill-switch, CAS single-writer, idempotency, dead-letter) still hold with the stages in place | Unit + Component | `wave3-consumer-wiring-frozen-order` | — | ✅ green (Layer-2 live: the factory-built consumer runs end-to-end over real stores + queue — Session 20 cont., `e520b33`) |
| **D102 / D101** | `build_promotion_plane` pins **one** candidate store across the scheduler + inbox so the promotion cron and the review inbox never diverge; the corpus scan + the deferred warehouse probe both **degrade fail-closed** (no dead-letter, no uncaught raise) so a corpus/warehouse hiccup can never crash the consumer or fake a merge | Unit + Component | `wave3-promotion-plane-shared-store-failclosed` | — | 🟡 unit-green (Layer-1 built; auto-promotion Layer-2 gated on the deferred real probe/resolver) |
| **D96 / D101 / D48** | Full-pipeline-live e2e: the **factory-built consumer** over real stores + real Redis queue processes a session end-to-end — a clean blueprint auto-lands `candidate` enriched + seeds the durable corpus `hit_count=1`, and a **second identical session increments to 2** (cross-session D48 accrual on real Couchbase); a `global_knowledge` candidate routes to `in_review`; an S5 `reroute` lands in the real `user_knowledge` store | Component | `wave3-pipeline-live-e2e` | — | ✅ green (Layer-2 live vs real Couchbase + Redis + neo4j — the crown-jewel proof, Session 20 cont., `e520b33`; Layer-3 conformance is a separate later concern) |
| **D17 / D95** | User-store-live: the entity-BEARING per-user `learning_user` store round-trips per-user (S8), enforces **cross-user isolation** (never surfaces another user's rows), and is **RBAC-denied on the other learning buckets** (audit/candidates/corpus) — proven against the live provisioned `learning_user` role | Component | `wave3-user-store-live` | — | ✅ green (Layer-2 live vs real Couchbase — Session 20 cont., `e520b33`) |

---

## Additional tagged invariants from the S9-activation arc (Track-B promotion scheduler REAL — the loop learns AND forgets, Session 21)

The load-bearing invariants of the S9-activation arc (3 slices, `f93fd1a` probe+resolver / `fa30990`
landing writer / `89e4694` retraction), see
[learning-loop-s9-activation-design.md](learning-loop-s9-activation-design.md). Before this arc the
promotion scheduler's write plane was three fail-closed stubs; now a validated candidate's blueprint is
LANDED into the neo4j retrieval corpus (recallable) and a demoted / leaked / user-corrected blueprint is
RETRACTED from recall — **the loop learns AND forgets end-to-end**. Rows are marked per honesty:
**`✅ green`** where a Layer-2 live test proves the invariant (probe 3/3, land+recall, retraction 7/1
against the real l2 stack), **`🟡 unit-green`** where only the Layer-1 leg is built. The loop is
production-activatable but **DORMANT** (`LEARNING_ENABLED` off; the write plane activates only when
neo4j + embedder + MCP + token config are all present) and **Layer-2-proven, not Layer-3-conformance-proven**.
Gates at arc close: **1901 passed / 111 skipped, ruff clean.**

| Decision | Invariant | Test layer | Test slug | Conformance scenario | Status |
|---|---|---|---|---|---|
| **D98** | The real `MCPWarehouseProbe` returns ONLY `(row_count, distinct_grain_count, columns)` — the replay SQL's rows are DISCARDED, the grain probe returns aggregates; no row value ever leaves the probe (no value oracle) | Unit + Component | `S9-probe-never-returns-value` | **No-silent verification** | ✅ green (Layer-2 live: real grain probe 5 rows / 5 distinct vs `dbpcm_warehouse.employee`, no value surfaced — `f93fd1a`) |
| **D98 / D57 / D89** | The probe travels the MCP `runQuery` choke point under a JWT minted for the blueprint's OWN `uses` (`column_scope == uses`, session-BOUND) — a probe reading outside `uses` is DENIED and holds `probe_unavailable`; D57 teeth reused verbatim so offline-replay-green ⇒ live-run-green over the exact enforced path. The offline grain SQL is built via the SHARED `runtime/blueprint/grain_probe.py` helper (function-IDENTITY-pinned to the live executor — cannot drift) | Unit + Component | `S9-probe-scoped-to-uses` | **Scope denial** | ✅ green (Layer-2 live: out-of-`uses` column denied `COLUMN_SCOPE_VIOLATION` by real D57 — `f93fd1a`) |
| **D80b** | An empty `uses` NEVER mints an allow-all token: `golden_replay` short-circuits to a clean `no_uses_scope` HOLD before any mint/MCP call, and `HttpTokenMinter.mint` independently RAISES on an empty scope (defense-in-depth) — asserts ZERO mint/MCP calls (review-caught safety hole #1) | Unit | `S9-empty-uses-refuses-allow-all` | — | 🟡 unit-green |
| **D101** | `CandidateStoreDependencyResolver.is_resolved` is fail-closed: a `depends_on` sibling that is missing / non-`validated` / store-error ⇒ unresolved (HOLD); only a `validated` sibling ⇒ resolved; no-deps ⇒ trivially resolved; never fail-open | Unit | `S9-resolver-fail-closed` | — | 🟡 unit-green |
| **D17 / D95** | Landing fires strictly on the `→ validated` edge; the `BlueprintSeed` is projected from generalized fields ONLY (template / resolves / result_grain / intent / slots) — never `evidence`, audit spans, or entity-bearing payload; the D17 entity last-gate asserts against PRE-strip spans captured by the scheduler (fixing the strip-blanks-spans provable-no-op — review-caught safety hole #2), so it fires through the real scheduler path (RAISES `LandingEntityError`, never lands) | Unit + Component | `S9-land-only-validated` | — | ✅ green (Layer-2 live: land a validated candidate then recall it via `Neo4jVectorIndex.recall` — `fa30990`) |
| **D95** | Land-then-status is load-bearing: "not landed ⇒ not validated" — a landing failure holds `landing_failed` and the candidate stays `candidate`/`in_review` (never a half state); a crash between land and the status write re-lands idempotently next cycle (deterministic `bp::{canonical_key}` id → `load_corpus` MERGE by id) then writes status; auto-promotion of a blueprint is gated on a real landing writer (`landing_unavailable` hold otherwise) | Unit + Component | `S9-land-then-status-idempotent` | — | ✅ green (Layer-2 live: second land of the same canonical_key MERGEs to exactly ONE `:Blueprint` node — `fa30990`) |
| **D43** | Blueprint recall filters ineligible nodes: `coalesce(status,'validated')='validated' AND coalesce(drift_status,'clean')<>'suspect'` — an EXPLICIT `candidate`/`retired`/`rejected` status OR a `suspect` drift is excluded from recall (the fail-CLOSED gate the moment the node is stamped) | Unit + Component | `S9-recall-filters-ineligible` | **Scope denial** | ✅ green (Layer-2 live: a demoted blueprint is NO LONGER recalled — `89e4694`) |
| **D43 / D87** | Recall seed-compat: the `coalesce(...)` defaults treat an ABSENT `status`/`drift_status` property as recallable, so a hand-authored seed node with no lifecycle stamp stays recallable; `_KNOWLEDGE_RECALL_QUERY` + `getBlueprint` are deliberately left unfiltered (only blueprints land via the loop) — the seed corpus recall is unregressed | Unit + Component | `S9-recall-seed-compat` | — | ✅ green (Layer-2 live: hand-authored seed corpus stays recallable throughout a demote — `89e4694`) |
| **D43** | Retraction write-back is FAIL-OPEN: demote / user-correction / reject / leak-retract edges stamp the landed node (`update_status`, MATCH-by-id) BEFORE the store transition — a neo4j hiccup swallows + LOUDLY logs and never blocks the authoritative store transition; a held-demoted (suspect) blueprint is RE-ASSERTED every cycle so a transient neo4j failure converges to non-recallable (closes the review-caught self-heal-only-safe-direction gap); write-back FLIPS the stamp (never `DETACH DELETE` — preserves provenance for incident response) | Unit + Component | `S9-demote-writes-back-failopen` | — | ✅ green (Layer-2 live: retraction 7/1 — demote excludes the node, seed unregressed — `89e4694`) |
| **D43 / D58** | The inbox leak-PULL (`ReviewInbox.retract` → `PromotionScheduler.apply_retract`, validated → retired) STAMPS the landed node `retired` before the store retire (closes the review-caught unwired-retract-edge gap); an UNWIRED inbox's default scheduler has no landing writer, so retract is store-only there (the stamp happens only when a real writer is wired). Physical index removal stays S10; the stamp closes the recall exposure now | Unit | `S9-retract-self-heals` | — | 🟡 unit-green |

---

## Additional tagged invariants from Session 22 (real-LLM extractor + learning/runtime observability, D25/D31)

The load-bearing NEW invariants of the Session-22 work — the extractor's forced-tool schema hardened

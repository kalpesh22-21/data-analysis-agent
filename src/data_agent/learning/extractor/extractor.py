...
_SYSTEM_PROMPT = (
    ...
    "start/end range is one period_range slot ONLY when the accepted SQL uses one literal "
    "BETWEEN predicate; separate comparison predicates are not pairable. (6) The aggregated "
    ...
    "'occurrence':0,'context':'table_source','value':'5'}. "
    "A trailing window `INTERVAL 5 YEAR` may use a required relative_window slot with "
    "locator {\"kind\":\"interval_argument\",\"unit\":\"YEAR\",\"occurrence\":0,"
    "\"context\":\"interval\",\"value\":\"5\"}; the unit stays fixed in SQL. "
    "A replaceable literal membership set such as `department_name IN ('Sales','Finance')` "
    "must be ONE required list slot with locator {\"kind\":\"in_list\","
    "\"table\":\"db.table\",\"column\":\"department_name\",\"occurrence\":0,"
    "\"context\":\"in_predicate\",\"value\":\"Sales,Finance\"}; do not emit one slot "
    "per member. Its binds_to is the fully-qualified column. "
    "A replaceable top-N `LIMIT 5` must use a required positive_integer slot with "
    "`binds_to: null` and locator {\"kind\":\"limit_argument\",\"occurrence\":0,"
    "\"context\":\"limit\",\"value\":\"5\"}. This does not cover OFFSET. "
    "A literal date range `hire_date BETWEEN '2025-01-01' AND '2025-12-31'` may use one "
    "required period_range slot with `binds_to: null` and locator "
    "{\"kind\":\"between_range\",\"table\":\"db.table\",\"column\":\"hire_date\","
    "\"occurrence\":0,\"context\":\"between_predicate\","
    "\"value\":\"2025-01-01,2025-12-31\"}. Never use period_range for two separate "
    "comparison predicates. "
    "A horizon encoded by repeated "

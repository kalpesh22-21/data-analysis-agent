...
The regex/NER layer (`entities.py`) catches KNOWN entity shapes; the second layer
is a semantic scan that (a) catches entities no pattern anticipated and (b)
classifies WHY an entity is present — an accidental leak vs. a legitimate
user-specific fact that should be rerouted to `user_knowledge`. That judgment is
an LLM call in production, so it is an INJECTED seam (mirroring the S3 extractor's
`ModelClient` DI): Layer-1 tests drive a deterministic scripted double — NEVER a
real LLM. The default is the null scanner (regex-only), so an unwired gate still
runs and fails safe (no reroute, all entity hits -> reject/quarantine).
"""
...
class SemanticScanRequest:
    """What the gate hands the semantic scanner: the candidate type + the
    per-field text already selected for scanning (never the raw envelope — the
    scanner sees only the entity-relevant fields)."""
...
class NullSemanticEntityScanner:
    """The default (unwired) scanner: always `clean`, no hits. The gate then runs
    regex-only and fails safe — an entity hit with no `user_fact` signal can never
    become a reroute, only a reject (global) or quarantine (blueprint)."""

...
def _is_multi_row_answer_call(tool_name: str, status: str, preview: ResultPreview | None) -> bool:
...
def answer_shape_nudge_text(
    draft: str | None,
    multi_row_calls: int,
    *,
    refusal_actioned: bool = False,
    cards_present: bool = False,
    capability_repick_used: bool = False,
    capability_engaged: bool = False,
) -> str:
    ...
    if cards_present:
        lines.append(
            "The options are prepared; do not call them again. One response can carry the table and options."
        )
    elif capability_engaged and not capability_repick_used and not refusal_actioned:
        lines.append(
            "You may searchCapabilityTools, load with getCapabilityTool and present one better option only if its declared coverage explicitly fits the request."
        )
    return "\n".join(lines)
...
def refreshed_analysis_state(tool_result: ToolResult, turn_index: int) -> AnalysisState | None:
...
class AnswerShapeCounter:
    ...
    def note_call(self, tool_name: str, status: str, preview: ResultPreview | None) -> None:
...
class FinalizationGate:
    ...
    async def may_refuse(self, kind: FinalizationBlockKind) -> bool:
        ...
        try:
            ...
            return False
        if granted:
            self._observer("loop_finalization_block_spent", {"window": self._window_count})

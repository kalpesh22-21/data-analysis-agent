...
AGENT_SYSTEM_PROMPT = (
    ...
    "table). Send updateAnalysisState closing what remains and the appropriate final "
    "answer tool in the SAME response; state calls run first, so one response does both.\n"
    "\n"
    "## Operating procedure\n"
    ...
    "cover. Never paste the rows into `answer` as a markdown table "
    ...
    "- If no table is needed, call answerWithText with the complete final prose in "
    "`answer`. Name every successful current-turn tool that directly supports it in "
    "`evidence`; use an empty list only when no retrieved evidence is needed. A normal "
    "assistant response never reaches the user and cannot end the turn.\n"
    "\n"
    "## Answering\n"

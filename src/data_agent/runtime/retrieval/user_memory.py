"""UserMemoryProvider — the user-memory pre-injection seam.

No user store exists yet, so this ships the seam plus a `NullUserMemoryProvider` that
returns `[]` — honest degradation, no fabricated defaults. The real provider lands when the
user store does.

User memory is NEVER a model-facing tool (it is personal and entity-bearing) — it reaches
the model only via runtime pre-injection.
"""

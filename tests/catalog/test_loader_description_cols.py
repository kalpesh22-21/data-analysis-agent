"""Unit tests for `load_description_cols_from_catalog()` / `_extract_description_cols`.

Layer: 1 — Unit (pure logic; hand-built catalog dicts for the projection edge cases
plus the committed export fixture for the shape/alignment checks; no ClickHouse).

`load_description_cols_from_catalog()` is a third projection of the same catalog dict
that `build_sqlglot_schema_from_catalog()` and `load_semantic_catalog_from_catalog()`
build from. It returns the AUTHORED {code_col -> sibling-label-col} linkage so
`resolveValues` can SELECT both columns jointly instead of relying on the
`_candidate_description_columns` naming convention (which case-mismatches, e.g.
`distributedDepartmentDescription`, and misses non-convention names, e.g.
`FieldId` -> `FieldLabel`).
"""
...
# ---------------------------------------------------------------------------
# 1. load_description_cols_from_catalog() shape — {db_table: {code_col: desc_col}}
# ---------------------------------------------------------------------------
...
_CODES_ENTRY = {
    ...
    "columns": {
        ...
        "FieldLabel": {"type": "Nullable(String)", "description": "The human-readable label."},
        "PlainValue": {
            "type": "Nullable(String)",
...
def test_load_description_cols_returns_declared_linkage() -> None:
    result = load_description_cols_from_catalog({"dbpcm_warehouse.codes": _CODES_ENTRY})
    assert result == {"dbpcm_warehouse.codes": {"FieldId": "FieldLabel"}}


def test_column_without_description_col_absent_from_inner_map() -> None:
    """A column that omits `description_col` must not appear in the inner map — it
    is not padded with a convention-guessed default."""
    inner = load_description_cols_from_catalog({"dbpcm_warehouse.codes": _CODES_ENTRY})[
        "dbpcm_warehouse.codes"
    ]
...
def test_table_with_no_links_present_as_empty_inner_map() -> None:
    """Every catalogued table is a key; a table with zero declared links maps to {}
    (kept for symmetry with the sqlglot-schema / semantic-overlay projections)."""
    plain = {
        "database": "dbpcm_warehouse",
        "table": "plain",
        "columns": {"A": {"type": "Nullable(String)"}, "B": {"type": "Nullable(String)"}},
    }
    result = load_description_cols_from_catalog({"dbpcm_warehouse.plain": plain})
...
def test_description_cols_keys_align_with_sqlglot_schema() -> None:
    """Same keying as the sqlglot-schema projection — one entry per table, never
    drifting from the schema view."""

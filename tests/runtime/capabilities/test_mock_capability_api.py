...
async def test_identifier_is_discoverable_and_translates_the_requested_parameters():
    client = HttpCapabilityClient(
        base_url="http://mock/v1/capabilities", transport=httpx.ASGITransport(app=app)
    )
    cards = await client.search(
        "Show me the SSN of every employee named Smith", ("data_widget",), end_user_jwt="mock-user"
    )
    ...
    schema = definition.tool_schema(
        CapabilityResolutionRegistry.load("config/capability-value-resolution.yaml")
    )
    ...
    assert "capability_ref in finalizeAnswer" in schema["description"]
...
async def test_identifier_mock_hydrates_references_without_fabricating_identifier_values():
    ...
    card = await client.hydrate(
        "get_employee_personal_identifier",
        query="Show identifiers for Smith",
        raw_arguments={"employees": ["Smith"], "department": ["Sales"]},
        end_user_jwt="mock-user",
        forward_end_user=True,
    )
    assert card["arguments"]["employees"] == [
        {"eecode": "SMITH", "description": "Smith", "entity_type": "employee"}
    ]

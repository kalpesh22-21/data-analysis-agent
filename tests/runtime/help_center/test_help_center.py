...
from data_agent.runtime.help_center.tools import SearchHelpCenterTool
...
async def test_search_reranks_candidates_and_returns_five() -> None:
    ...
    tool = SearchHelpCenterTool(client=FakeHelpCenter(), reranker=reranker)
...
async def test_http_contract() -> None:
    ...
    hits = await client.search("question", 25)
    document = await client.get_document(hits[0].id)
